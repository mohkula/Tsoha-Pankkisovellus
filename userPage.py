...
def ShowUserInfo():
    ...
    if session["username"]:
        ...
        if session["csrf_token"] == request.form["csrf_token"]:
            ...
            user_infoList = []
            for i in user_info:
                user_infoList.append("Käyttäjänimi: " + str(i[0]) +
                                     ", Sähköpostiosoite: " + str(i[1]) +
                                     ", Puhelinnumero " + str(i[2]) + ", Osoite: "
                                     + str(i[3]))
...
def showCards():
    if session["username"]:
        if session["csrf_token"] == request.form["csrf_token"]:
            ...
            sql = """SELECT card_number, expirationdate FROM cards WHERE customer_id=
                (SELECT id FROM users WHERE username =:username) AND active = TRUE"""
            result = db.session.execute(sql, {"username":username})
            ...
            card_infoList = []
            for i in card_info:
                card_infoList.append("Kortin numero: " +str(i[0]) +
                                     ", kortin voimassaoloaika: " + str(i[1]))
...
def userHasOrderedCard(username):
    sql = """Select customer_id FROM orders WHERE customer_id =
        ...
        if session["csrf_token"] == request.form["csrf_token"]:
            ...
            if not orderCard(username):
                return render_template("userPage.html",
                                       error="""Voit tilata yhden
                                       kortin kerrallaan, odota hyväksyntää""")
            ...
            sql = """INSERT INTO cards (customer_id, account_id, card_number)
                VALUES (:customer_id,  (SELECT id from bankAccounts WHERE customer_id=
                (SELECT id FROM users WHERE username =:username))  ,:card_number)"""
            ...
            sql = """UPDATE cards SET expirationdate = expirationdate + interval '4 years'
    ...
    if session["username"]:
        ...
        if session["csrf_token"] == request.form["csrf_token"]:
            ...
            if not card_number.isdigit():
                return render_template("reportMissingCard.html",
                                       error="Antamasi kortin numero ei kelpaa")



            if not isUsersCard(card_number):
                return render_template("reportMissingCard.html",
                                       error="Korteistasi ei löydy korttia numerolla: "
                                       + card_number)
...
def showBankAccount():
    if session["username"]:
        if session["csrf_token"] == request.form["csrf_token"]:
            ...
            sql = """SELECT account_number, balance, openingdate
            FROM bankAccounts WHERE customer_id=
            (SELECT id FROM users WHERE username =:username) AND active = TRUE"""
            result = db.session.execute(sql, {"username":username})
            ...
            bank_infoList = []
            for i in bank_info:
                bank_infoList.append("tilinumero: " +str(i[0]) + ", saldo: "
                                     + str(i[1]) + ", avattu: " + str(i[2]))
